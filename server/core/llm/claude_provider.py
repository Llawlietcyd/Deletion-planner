"""Claude (Anthropic) provider — real API integration for plan decisions and deletion reasoning."""
...
3. Respect the user's capacity constraints — never plan more work than fits.
...
class ClaudeLLMService(BaseLLMService):
    """Real Anthropic Claude API provider for AI-powered planning decisions."""
    ...
    def __init__(self, lang: str = "en"):
        ...
        if not self.api_key:
            logger.warning("ANTHROPIC_API_KEY not set — Claude provider will fall back to mock.")

    def _lang_instruction(self) -> str:
        if self.lang == "zh":
            return "Respond in Chinese (简体中文)."
        return "Respond in English."

    def _call_claude(self, system: str, user: str, max_tokens: int = 1024) -> str:
        """Make a synchronous HTTP call to the Anthropic Messages API."""
    ...
    def recommend_decisions(self, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        deletion_info = []
        for c in rule_snapshot.get("deletion_candidates", []):
            deletion_info.append(f'id={c["task_id"]} reasons={c.get("rule_reasons", [])}')
        ...
        try:
            ...
            logger.error("Claude recommend_decisions failed: %s", e)
            return self._fallback.recommend_decisions(context)
    ...
    def generate_deletion_reasoning(
        ...
        try:
            return self._call_claude(SYSTEM_PROMPT, prompt, max_tokens=256)
        except Exception as e:
            logger.error("Claude deletion reasoning failed: %s", e)
            return self._fallback.generate_deletion_reasoning(task, rule_reasons)
