"""OpenAI provider — real API integration for plan decisions and deletion reasoning."""
...
class OpenAILLMService(BaseLLMService):
    """Real OpenAI API provider for AI-powered planning decisions."""
    ...
    def __init__(self, lang: str = "en"):
        ...
        if not self.api_key:
            logger.warning("OPENAI_API_KEY not set — OpenAI provider will fall back to mock.")
    ...
    def _call_openai(self, system: str, user: str, max_tokens: int = 1024) -> str:
    ...
    def recommend_decisions(self, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error("OpenAI recommend_decisions failed: %s", e)
            return self._fallback.recommend_decisions(context)
    ...
    def generate_deletion_reasoning(
        ...
        try:
            return self._call_openai(SYSTEM_PROMPT, prompt, max_tokens=256)
        except Exception as e:
            logger.error("OpenAI deletion reasoning failed: %s", e)
            return self._fallback.generate_deletion_reasoning(task, rule_reasons)
